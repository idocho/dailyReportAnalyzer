...
| 버전 | 날짜 | 내용 |
|------|------|------|
| 3.0 | 2026-06-14 | **v0.5 — 락인 지향 산출물 개편**. 리포트의 1차 목적을 분기말 학부모 대상 **홍보·리텐션**으로 명시(§1.1). 정확도는 목적이 아니라 신뢰의 연료 — 거짓 없이 선별·재구성. ① **moment 원재료 전달**: `momentSeeds`(하이라이트 날짜 + 실제 태그 라벨) AI 전달 — 날짜만 주던 구조에서 환각 제거, 원재료 없으면 빈 배열 강제. ② **성장 비교 블록**: 이미 계산되던 `prevAxes`(비포)를 프롬프트·리포트에 노출 — eval/cover 성장 서사 + 상단 **성장 콜아웃 배너**(가장 크게 오른 역량 ▲). ③ **약점→관리증거 전환**: caution을 숨기기만 하던 것에서 next_points 학원 액션("저희가 지도 중")으로 재배선. ④ **성적 비대칭**: 평균 이상만 등수·평균선 노출, 미달은 등수 숨김(프롬프트+`buildReportScoreBars`). ⑤ **캘린더 부진일 분홍 제거**: low·bad→무채색 흡수. ⑥ **섹션 재배치**: 감정(eval·성장배너·moments·키워드)→근거(레이더·달력·태그·성적)→미래(집중포인트)→맺음. ⑦ **브랜딩·CTA**: 설정에 학원명·대표교사명·맺음말 CTA(`drw_academy_name`/`drw_teacher_name`/`drw_report_cta`) — 하드코딩 '수학학원'·'선생님' 폴백 대체. ⑧ 키워드 비면 폴백 |
...
> **산출물 1차 목적 (v0.5 명문화)**: 분기 막바지 학부모 대상 **홍보·리텐션(락인)**.
> "자녀가 체계적으로 관리되며 성장 중"이라는 인상 형성이 목표이며, 객관적 성적표가 아니다.
> 단, **정확도는 버리는 것이 아니라 신뢰의 연료**다 — 빈 칭찬/템플릿은 신뢰를 깨 락인을 역행시키므로,
> 실제 관찰 데이터를 **거짓 없이 선별·재구성**해 구체적 성장 서사를 만든다.
> 설계 원칙: ① 구체성(날짜·태그 기반 실제 장면) ② 성장 가시화(비포→애프터) ③ 약점은 은폐가 아니라 "관리 중" 증거로 전환 ④ 부진 지표(등수·부진일)는 노출 최소화.
...
v0.5에서 **감정→근거→미래→맺음** 순으로 재배치 (감정 무기를 상단으로):

```
[커버]        학원명(설정) · 학생명 · 반/기간 · AI 커버 문장
[01]          선생님이 본 이번 달 (AI 서술 2~3문장)        ┐ 감정
[성장배너]    비교기간 대비 가장 크게 오른 역량 ▲ (조건부)  │
[02]          선생님이 기억하는 순간 (moment 원재료 기반)   │
[03]          이달의 키워드 (비면 폴백)                     ┘
[04]          역량 레이더 (5각형 · 비포/애프터 오버레이)    ┐ 근거
[05]          한 달 수업 컨디션 (날짜별 달력, 부진일 무채색) │
[06]          이달에 보여준 것들 (긍정 태그 횟수 바)        │
[07]          성적 추이 (평균 이상만 등수·평균선 노출)      ┘
[08]          다음달 집중 포인트 (학원 액션 ≥1 포함)        ← 미래
[푸터]        선생님 한마디 · CTA(설정) · 학원명 · 담임 · 발송일  ← 맺음
```

> 성장배너: `radarAxesPrev` 대비 +3 이상 오른 축을 초록 배너로 강조(없으면 생략).
> moment: AI는 `momentSeeds`(하이라이트 날짜+태그) 원재료만 사용, 없으면 빈 배열.
> 캘린더: low·bad 컨디션은 분홍 강조 대신 normal(무채색)로 흡수.
> 성적: 평균 이상(`myPct>=avgPct`)일 때만 등수·평균선 노출, 미달은 점수만.
...
**프롬프트 입력 (v0.5 추가)**:
- `momentSeeds` — 하이라이트 날짜 + 실제 태그 라벨. moments는 이 원재료만 사용(환각 방지).
- 성장 추이 — `prevAxes` 대비 축별 변화량. eval/cover 성장 서사 재료.

**caution 처리 (v0.5 — 약점→관리증거)**:
- 부정적 직접 표현 금지는 유지하되, 약점은 next_points의 **학원 액션**("저희가 ~ 지도 중")으로 흡수.
- `late`: "시간 약속 신경" 수준 완곡 (구버전 직접 언급 → 완곡으로 하향)
- `sleepy` / `chat` / `attitude`: 완곡 + 관리 프레임만

**성적 표현 비대칭 (v0.5)**: 평균 이상·상위·향상은 구체 부각(점수 수치 본문 직접 금지는 유지), 평균 미달·하락은 등수/미달 비노출·과정 중심 완곡.

**next_points (v0.5)**: 최소 1개는 학원 직접 지도 계획(관리 가치 각인 → 재등록 동기), 나머지는 가정 응원.
